...
presetup() {
    ...
    pushd ./artifacts/src/github.com/loyaltyToken/go
     go mod tidy
...
VERSION="1"
CC_SRC_PATH="./artifacts/src/github.com/loyaltyToken/go"
CC_NAME="loyaltyToken"
...
chaincodeInvoke() {
    ...
    ## Add private data
    export TOKEN=$(echo -n "{\"key\":\"1111\", \"name\":\"Sysco-Foody-coin\",\"type\":\"Food coins\",\"rate\":\"15 coins =1 token\",\"owner\":\"pavan\",\"value\":\"10000\"}" | base64 | tr -d \\n)
    peer chaincode invoke -o localhost:7050 \
        --ordererTLSHostnameOverride orderer.example.com \
        --tls $CORE_PEER_TLS_ENABLED \
        --cafile $ORDERER_CA \
        ...
        --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_ORG1_CA \
        --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_ORG2_CA \
        -c '{"function": "createPrivateToken", "Args":[]}' \
